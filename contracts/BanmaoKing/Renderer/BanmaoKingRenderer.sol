...
contract BanmaoKingRenderer is IBanmaoKingRenderer {
    ...
    function renderSVG(uint256 tokenId, BanmaoKingTraits calldata traits_) public view returns (string memory) {
        ...
        return string.concat(
            ...
            tokenId.toString(), '">', _background(traits_.background),
            shadow, group, bodyLib.render(traits_.body),
            '<g>', expressionLib.render(traits_.expression), accessoryLib.render(traits_.accessory), '</g>',
            '</g></svg>'
        );
    }
    ...
    function _background(uint8 id) private pure returns (string memory) {
        if (id == 0) return '<rect width="512" height="512" fill="#f4efe7"/><ellipse cx="256" cy="477" rx="101" ry="13" fill="#625b52" opacity=".18"/>';
