 ...
 * @dev V2: Updated multipliers for ~95% RTP (house profits 5%)
 */
contract BanmaoSlotsMultiPoolV2 is Ownable, ReentrancyGuard, Pausable {
    ...
    // V2: Updated for ~95% RTP (house profits 5%)
    // Old values had ~108% RTP (house loses money)
    uint16[3][6] public payoutMultipliers = [
        [900, 4500, 17500],   // Banmao: 9x, 45x, 175x (was 10x, 50x, 200x)
        [700, 3500, 12500],   // Banana: 7x, 35x, 125x (was 8x, 40x, 150x)
        [450, 1700, 7000],    // Diamond: 4.5x, 17x, 70x (was 5x, 20x, 80x)
        [250, 1300, 4500],    // Star: 2.5x, 13x, 45x (was 3x, 15x, 50x)
        [180, 700, 2200],     // Clover: 1.8x, 7x, 22x (was 2x, 8x, 25x)
        [130, 450, 1300]      // Seven: 1.3x, 4.5x, 13x (was 1.5x, 5x, 15x)
    ];
    
    uint16 public jackpotMultiplier = 45000; // 450x for jackpot (was 500x)
    ...
    event SpinRevealed(uint256 indexed poolId, address indexed player, uint8[5] result, uint256 payout, bool isJackpot, bytes32 seed);
    ...
    function revealSpin(bytes32 seed) external nonReentrant whenNotPaused {
        ...
        // Security: Require 2 block delay to prevent miner manipulation
        if (block.number < commit.blockNumber + 2) revert WaitBlock();
    ...
    function _performReveal(
        ...
        emit SpinRevealed(vars.poolId, player, vars.result, vars.payout, vars.isJackpot, seed);
    }
    ...
    function _performMultiReveal(
        ...
        for (uint256 i = 0; i < v.spinCount; i++) {
            // Block scope to limit stack usage
            {
                ...
                emit SpinRevealed(v.poolId, player, result, spinPayout, isJackpot, spinSeed);
            }
        }
    ...
    /**
     * @notice Verify a spin result off-chain (for transparency)
     * @dev Allows anyone to independently verify that results match the contract logic
     * @param seed The revealed seed used for the spin
     * @param blockNumber The block number when the spin was committed
     * @param player The player address
     * @param poolId The pool ID
     * @return result The 5 symbol results
     */
    function verifySpinResult(
        ...
        address player,
        uint256 poolId
    ) external view returns (uint8[5] memory result) {
        return _generateResult(seed, blockNumber, player, poolId);
    }
