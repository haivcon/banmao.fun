...
async function main(){
 const sources={};for(const f of ['BanmaoKingExpressionLib','BanmaoKingBodyLib','BanmaoKingBackgroundEffects']){const p=`contracts/BanmaoKing/Lib/${f}.sol`;sources[p]={content:fs.readFileSync(path.join(root,p),'utf8')};}
 ...
 for(const [name,a]of Object.entries(artifacts)){const runtime=a.evm.deployedBytecode.object.length/2,init=a.evm.bytecode.object.length/2;assert(runtime<=24576&&init<=49152,`${name}: runtime=${runtime}, init=${init}`);if(name.includes('Expression'))console.log(name,runtime,init);}
 ...
 async function deploy(name,args=[]){const a=artifacts[name],factory=new ethers.ContractFactory(a.abi,a.evm.bytecode.object,provider.getSigner());const tx=factory.getDeployTransaction(...args);assert((tx.data.length-2)/2<=49152, name+' init + arguments');return(await factory.deploy(...args,{gasLimit:90000000})).deployed();}
 const parts=[];for(let i=0;i<3;i++)parts.push(await deploy('BanmaoKingExpressionPart'+i));
 const expression=await deploy('BanmaoKingExpressionLib',parts.map(p=>p.address));
 ...
 const body=await deploy('BanmaoKingBodyEffects'),bg=await deploy('BanmaoKingBackgroundEffects'),cyborg=await deploy('BanmaoKingCyborgBody');
