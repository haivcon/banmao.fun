...
function generate(name, entries, groups, royal = false) {
  ...
  const canonicalSakura = name === 'BanmaoKingBackgroundExpansion';
  const parts = groups.map((ids, i) => canonicalSakura && ids.length === 1 && ids[0] === 14
    ? `contract ${name}Part${i} { error InvalidTrait(); function render(uint8 id) external pure returns(string memory) { if(id!=14)revert InvalidTrait(); return BanmaoKingSakuraGarden.render(); } }\n`
    : contract(name+'Part'+i, entries.filter(([id]) => ids.includes(id)))).join('');
  ...
  return imports + (canonicalSakura ? require('./king-canonical-sections.cjs').section('contracts/BanmaoKing/Lib/BanmaoKingBackgroundExpansion.sol','SakuraGarden') : '') + parts + `contract ${name} {\nerror InvalidTrait();\n${fields}\n${royal?'BanmaoKingRoyalBackground public immutable royal;':''}
