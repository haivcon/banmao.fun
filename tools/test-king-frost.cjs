...
const root=path.resolve(__dirname,'..'),dir=path.join(root,'test-results/frost');fs.mkdirSync(dir,{recursive:true});
...
const garden=require(path.join(root,'app/collection/banmaoking/frost-suit-contract.json'));
const wrap=s=>'<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512" viewBox="0 0 512 512">'+s+'</svg>';
const scenes=[wrap(garden),...[0,7,19,20].map(accessory=>wrap(previewSvg({body:14,expression:0,accessory,background:14},0,'s'+accessory)))];
...
const glow=svg.querySelector('[data-frost-glow]');ok(glow,'Frost glow');
...
document.querySelector('#result').textContent='PASS: standalone image playback ('+changed+' changed pixels), Frost effects and four full character compositions decoded';
