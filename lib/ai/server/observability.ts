...
export function safeLogRecord(input: Record<string, unknown>){
  const allowed = ["requestId", "model", "surface", "status", "durationMs", "errorCode", "toolName", "toolDurationMs", "toolStatus", "personaVersion", "ragMode", "ragHitCount", "retryCount", "voiceWarningCount"] as const;
