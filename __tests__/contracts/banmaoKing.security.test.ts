...
describe("BanmaoKing immutable on-chain release", () => {
  ...
  test("matches the approved refined SVG with actual Solidity render output", async () => {
    const approved = readFileSync(join(process.cwd(), "__tests__/fixtures/banmaoking-smile-refined.svg"), "utf8");
    const svg: string = await renderer.renderSVG(0, { body: 0, expression: 0, accessory: 0, background: 0 });
    expect(svg).toContain(bodySvg("#ffe53b", "#d9ad14").replace(/>\s+</g, "><"));
    expect(svg).toContain(expressionSvg(0));
    const expected = await sharp(Buffer.from(approved)).ensureAlpha().raw().toBuffer();
    const actual = await sharp(Buffer.from(svg)).ensureAlpha().raw().toBuffer();
    expect(actual.equals(expected)).toBe(true);
    if (process.env.BANMAOKING_QA_DIR) {
      const { writeFileSync } = await import("node:fs");
      writeFileSync(join(process.env.BANMAOKING_QA_DIR, "solidity.svg"), svg);
      await sharp(Buffer.from(svg)).png().toFile(join(process.env.BANMAOKING_QA_DIR, "solidity.png"));
      await sharp(Buffer.from(approved)).png().toFile(join(process.env.BANMAOKING_QA_DIR, "approved.png"));
    }
  });
  ...
  test("renders every catalogue entry and rejects out-of-range traits", async () => {
    ...
    for (let body = 0; body < 8; body += 1) {
      ...
      const normalizedBody = bodySvg(palette.color, palette.shade).replace(/>\s+</g, "><");
      expect(normalizedSvg).toContain(normalizedBody);
      expect(normalizedBody).toContain("M318 382c29-2 39 25 61 30 22 5 41-8 42-27");
      expect(normalizedBody).toContain("M178 418l-2 39c-16 6-24 19-19 29");
      expect(normalizedBody).toContain("M171 301c-27 7-48 29-49 53-1 20 13 34 30 28");
      expect(normalizedBody).toContain('transform="translate(318 405) scale(1.07) translate(-318 -382)"');
      expect(normalizedBody).toContain("M342 387q9 15 20 20M362 402q9 10 19 12M385 405q9 5 18 3");
      expect(normalizedBody).toContain("M162 477q22-9 45 0M305 477q23-9 45 0");
      expect(normalizedBody).toContain("M143 330q16 3 32 12M139 347q15 4 30 13");
      ...
      expect(normalizedBody.indexOf('id="cat-behind"')).toBeLessThan(normalizedBody.indexOf('id="banana-shell"'));
      expect(normalizedBody.indexOf('id="banana-shell"')).toBeLessThan(normalizedBody.indexOf('id="cat"'));
    }
    for (let expression = 0; expression < 12; expression += 1) {
      const svg = await renderer.renderSVG(1, { ...base, expression });
      expect(svg).toContain(expressionSvg(expression));
    }
