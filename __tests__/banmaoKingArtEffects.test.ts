...
describe('Banmao King authored art effects', () => {
  ...
  test('diamond facets have five to ten deterministic outward rays per eye', () => {
    ...
    for (let tokenId=0; tokenId<36; tokenId++) {
      const svg=previewSvg({body:7,expression:13,accessory:0,background:1},tokenId,'rays');
      ...
      expect(svg).toBe(previewSvg({body:7,expression:13,accessory:0,background:1},tokenId,'rays'));
    }
  ...
  test('diamond light packets travel outward and fully extinguish between bursts', () => {
    const svg=previewSvg({body:7,expression:13,accessory:0,background:1},1,'burst');
  ...
  test.each([1,9,10,11,12,15])('suit %i has a compact glowing badge with accessory-aware placement', body => {
    for(let accessory=0;accessory<=20;accessory++) {
      const svg=previewSvg({body,expression:0,accessory,background:1},1,'suit');
      expect(svg).toContain('scale(.3333333333)');
      expect(svg).toContain('data-suit-aura="true"');
      expect(svg).toContain(`data-accessory="${accessory}"`);
      expect(svg).toContain('transform:translate(256px,410px)');
  ...
  test('Cyborg keeps ordinal and locally articulated mechanical limbs', () => {
    expect(BODY_TRAITS[7].name).toBe('Cyborg Suit');
    const body=bodySvg(BODY_TRAITS[7].color,BODY_TRAITS[7].shade);
  ...
  test.each(Array.from({length:21},(_,i)=>i))('Cyborg expression %i keeps valid scoped SVG', expression => {
    const svg=previewSvg({body:7,expression,accessory:0,background:expression%17},1,'art');
    const ids=[...svg.matchAll(/\bid="([^"]+)"/g)].map(m=>m[1]);
    expect(new Set(ids).size).toBe(ids.length);
    for(const [,ref]of svg.matchAll(/(?:href="#|url\(#)([^"\)]+)/g)) expect(ids).toContain(ref);
  });
  test.each(Array.from({length:17},(_,i)=>i))('body/background %i is identifiable and animated', id => {
