...
export function RendererArtworkPreview({ assets, creator, createdAt, unlockTime, tier, batchPosition }: {
  ...
  const wallet = creator?.toLowerCase() ?? "CONNECT WALLET TO PREVIEW CREATOR";
  return (
    <svg className="box-renderer-preview" viewBox="0 0 600 600" role="img" aria-label="BanmaoBox sealed treasury preview">
      ...
      <g transform="scale(0.75)">
        ...
        {assets.map((asset,index)=><g key={`ledger-${asset.token}-${index}`}><text className="preview-mono preview-white" x="48" y={654+index*25} fontSize="12" fontWeight="700">{asset.token.toLowerCase()}</text><text className="preview-mono preview-white" x="560" y={654+index*25} textAnchor="end" fontSize="14" fontWeight="700">{rendererDisplayAmount(asset.amount,asset.decimals)}</text><text className="preview-mono preview-white" x="752" y={654+index*25} textAnchor="end" fontSize="13">{asset.symbol} / d{asset.decimals}</text><path d={`M48 ${662+index*25}H752`} stroke="#D8B565" strokeOpacity=".12"/></g>)}
      </g>
    </svg>
  );
}
