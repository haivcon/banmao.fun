...
export function CollectionDetailClient({ address }: { address: string }) {
  ...
  const [sourceVerification, setSourceVerification] = useState<"idle" | "pending" | "verified" | "unavailable" | "failed">("idle");
  const verificationRequest = useRef<BanmaoBoxVerificationRequest | null>(null);
  ...
  const verifySource = () => {
    verificationRequest.current?.cancel();
    setSourceVerification("pending");
    const request = requestBanmaoBoxVerification(item.transactionHash, (update) => {
      const outcome = classifyBanmaoBoxVerification(update);
      if (outcome === "success") setSourceVerification("verified");
      else if (outcome === "progress") setSourceVerification("pending");
      else if (outcome === "degraded") setSourceVerification("unavailable");
      else setSourceVerification("failed");
    });
    verificationRequest.current = request;
    void request.promise.finally(() => {
      if (verificationRequest.current === request) verificationRequest.current = null;
    });
  };
  const sourceMessage = sourceVerification === "verified" ? copy.sourceVerified
    : sourceVerification === "pending" ? copy.verifyQueued
      : sourceVerification === "unavailable" ? copy.verifyUnavailable
        : sourceVerification === "failed" ? copy.verifyFailed : null;
