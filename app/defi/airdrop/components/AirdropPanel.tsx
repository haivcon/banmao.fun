...
export default function AirdropPanel({ t, lang, playClick, playHover, playSuccess, playError }: AirdropPanelProps) {
    ...
    // Supports MULTI-SELECT: user can check multiple addresses and delete them all at once
    // Returns: single address string, "AUTO", comma-separated multi addresses ("MULTI:0x...,0x..."), or null
    const askFlaggedAddress = (batchSize: number, labels: Record<string, string>, batchAddrs?: string[]): Promise<string | null> => {
        return new Promise((resolve) => {
            ...
            document.getElementById("okx-flag-submit")!.onclick = () => {
                const inp = document.getElementById("okx-flag-input") as HTMLInputElement;
                // Multi-select mode: if checkboxes are selected, return them all
                if (selectedSet.size > 0) {
                    overlay.remove();
                    resolve("MULTI:" + Array.from(selectedSet).join(","));
                    return;
                }
                // Single paste mode fallback
    ...
    const executeAirdrop = async (retryEntries?: RecipientEntry[]) => {
        ...
        if (okbNum < gasNeeded) {
            // Hard bypass: We only warn, and force execution. Viem Wagmi estimation will be bypassed below by supplying gas explicitly.
            ...
            try {
                ...
                if (currentAllow < totalWei) {
                    ...
                    await writeContractAsync({
                        ...
                        // Force bypass local estimate to let user wallet handle approval errors 
                        gas: BigInt(60000) 
                    } as any);
                        ...
                        try {
                            ...
                            if (isEqual) {
                                ...
                                hash = await writeContractAsync({
                                    ...
                                    gas: BigInt(GAS_BATCH_BASE + addrs.length * GAS_PER_BATCH_RECIPIENT),
                                } as any);
                            } else {
                            ...
                            if (isUserReject || isLegalRisk) {
                                ...
                                const flagLabels = {
                                    ...
                                    search: t("searchWallet"), selectAll: t("flagSelectAll"), flagSelected: t("flagSelected"),
                                };
                                ...
                                // Option A-1: Multi-select mode — user checked multiple addresses via checkboxes
                                if (userInput && userInput.startsWith("MULTI:")) {
                                    const multiAddrs = userInput.slice(6).split(",").map(a => a.toLowerCase().trim()).filter(a => /^0x[a-fA-F0-9]{40}$/i.test(a));
                                    if (multiAddrs.length > 0) {
                                        const newBlacklist = new Set(blacklist);
                                        try {
                                            const stored: string[] = JSON.parse(localStorage.getItem(STORAGE_BLACKLIST) || "[]");
                                            for (const addr of multiAddrs) {
                                                newBlacklist.add(addr);
                                                if (!stored.includes(addr)) stored.push(addr);
                                                results.push({ address: addr, amount: amountPerWallet || "0", success: false, error: `⚠️ OKX Legal Risk — ${t("autoBlacklisted") || "auto-blacklisted"}` });
                                            }
                                            localStorage.setItem(STORAGE_BLACKLIST, JSON.stringify(stored));
                                        } catch(e) {}
                                        setBlacklist(newBlacklist);
                                        const multiSet = new Set(multiAddrs);
                                        batch = batch.filter(e => !multiSet.has(e.address.toLowerCase()));
                                        setSendResults([...results]);
                                        showToast(`🚫 ${t("multiDeletedCount") || "Removed"}: ${multiAddrs.length} ${t("airdropWallets") || "wallets"}`);
                                        consecutiveRejects = 0;
                                        retries++;
                                        if (batch.length === 0) break;
                                        continue;
                                ...
                                // Option A-2: User pasted a specific single address
                                if (userInput && userInput !== "AUTO" && !userInput.startsWith("MULTI:")) {
    ...
    const retryFailed = () => {
        // Filter out blacklisted addresses from retry list — ensures deleted wallets never come back
        const f = sendResults.filter(r => !r.success && !blacklist.has(r.address.toLowerCase()));
        if (f.length) {
            executeAirdrop(f.map(r => ({ address: r.address, amount: r.amount })));
        } else {
            showToast(t("retryAllBlacklisted") || "All failed addresses are blacklisted. Nothing to retry.");
        }
    };
                                        ...
                                        onClick={async () => {
                                            ...
                                            try {
                                                const txHash = await writeContractAsync({
                                                    ...
                                                    functionName: "approve",
                                                    args: [AIRDROP_CONTRACT, BigInt("0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff")],
                                                    gas: BigInt(60000) // Force Metamask popup
                                                } as any);
