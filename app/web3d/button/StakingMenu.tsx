...
export function StakingMenu({ position }: StakingMenuProps) {
    ...
    const hoverColor = '#7dd3fc';       // Light cyan on hover
    const glowPurple = '#a855f7';       // Purple glow accent
    const accentColor = '#06b6d4';      // Teal accent
    ...
    return (
        <group ref={groupRef} position={position} onClick={handleClick} scale={0.01}>
            <Billboard>
                {/* Animated outer glow ring - purple accent */}
                <RoundedPlane
                    width={btnWidth + 0.2}
                ...
                >
                    <meshBasicMaterial
                        color={isHovered ? hoverColor : glowPurple}
                        transparent
                ...
                <Html center position={[0, 0, 0.02]} style={{ pointerEvents: 'none' }} distanceFactor={8}>
                    <div style={{
                        ...
                        <span style={{
                            ...
                            fontWeight: 'bold',
                            textShadow: `1px 1px 0 #000, -1px -1px 0 #000, 1px -1px 0 #000, -1px 1px 0 #000, 0 0 ${isHovered ? '12px' : '8px'} rgba(0,212,255,0.5)`,
                            letterSpacing: '0.5px',
...
// Sparkle particles component - cyan/purple theme
...
function IdleSparklesX({ width, height }: { width: number; height: number }) {
    ...
    // Sparkle colors - cyan/purple/white mix for staking theme
    const colors = ['#00d4ff', '#a855f7', '#ffffff', '#06b6d4', '#c084fc', '#ffffff', '#00d4ff', '#a855f7'];
...
function BorderOrbitSparkles({ width, height }: { width: number; height: number }) {
    ...
    // Sparkle colors - cyan/purple gradient for staking
    const colors = ['#00d4ff', '#a855f7', '#ffffff', '#7dd3fc', '#c084fc', '#06b6d4'];
