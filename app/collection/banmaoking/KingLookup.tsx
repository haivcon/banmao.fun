...
export default function KingLookup({ lang }: { lang: Lang }) {
  ...
  return <section id="king-lookup" className="king-section king-mint-box">
    ...
    {result && <div className="king-lookup-result">
      ...
      <div className="king-lookup-details">
        ...
        <div className="king-nft-actions">
          <button className="king-nft-action" type="button" onClick={() => void copyKingAddress(result.id.toString(), lang)}><Copy size={16} aria-hidden="true" /><span>{kingLookupCopy(lang, "Copy ID")}</span></button>
          <button className="king-nft-action" type="button" onClick={() => void copyKingAddress(new URL(kingSharePath(result.id), window.location.origin).href, lang)}><Link2 size={16} aria-hidden="true" /><span>{kingLookupCopy(lang, "Copy link")}</span></button>
          <a className="king-nft-action king-nft-action-featured" href={kingSharePath(result.id)}><Link2 size={16} aria-hidden="true" /><span>{kingLookupCopy(lang, "NFT permalink")}</span></a>
        </div>
        ...
        <details key={result.id.toString()} className="king-lookup-technical">
          ...
          <p>Contract: <a href={xLayerExplorerUrl("address", kingAddress, lang)} target="_blank" rel="noopener noreferrer">{kingAddress} <ExternalLink size={14} aria-hidden="true" /></a></p>
          <div className="king-nft-actions">
            <a className="king-nft-action" href={result.image} download={`BanmaoKing-${result.id}-preview.svg`}><Download size={16} aria-hidden="true" /><span>{kingLookupCopy(lang, "Download preview SVG with composition code")}</span></a>
            <a className="king-nft-action" href={result.metadata.image} download={`BanmaoKing-${result.id}-original.svg`}><Download size={16} aria-hidden="true" /><span>{kingLookupCopy(lang, "Download original on-chain SVG")}</span></a>
          </div>
        </details>
    ...
    {query && <KingMetadataTools lang={lang} tokenId={refreshId} loading={status === 'loading'} allowExplorer={status === 'ready' || status === 'error'} onReload={() => setRetry(n => n + 1)} />}
  </section>;
}
