...
export default function KingMintResult({ lang, status, minted, batchSummary, hash, message, tokenId, onSelect }: {
  ...
  const t = KING_T[lang];
  const groups = status === 'success' ? groupMintResults(minted) : [];
  ...
  return <section className="king-transaction-result" data-tone={positive ? 'success' : negative ? 'error' : 'neutral'} aria-label={title}>
    ...
    {groups.length > 0 && <>
      ...
      <ul id={listId} className="king-result-wallets" aria-label={copy.recipients}>
        {(expanded ? groups : groups.slice(0, 3)).map(group => <li key={group.to.toLowerCase()}>
          ...
          <details><summary>{copy.details}</summary><code className="king-result-full-address">{group.to}</code></details>
    ...
    {hash && <a className="king-result-explorer" href={xLayerExplorerUrl('tx', hash, lang)} target="_blank" rel="noopener noreferrer"><ExternalLink size={16} aria-hidden="true" />{t.transaction}<code>{hash.slice(0, 10)}…{hash.slice(-8)}</code></a>}
  </section>;
}
