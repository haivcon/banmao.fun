...
export function previewSvg(traits: BanmaoKingTraitSelection, tokenId: number, prefix: string): string {
  ...
  scene += backgroundEffects(traits.background);
  scene += actionShadowSvg(traits.expression);
  ...
  scene += `<g${traits.background === 0 ? '' : ' filter="url(#king-shadow)"'}><g id="king-action-root" data-action="${traits.expression}" transform="${actionTransform(traits.expression)}"><g class="king-character-motion" data-accessory="${traits.accessory}">${(traits.accessory === 20 ? royal.REAR : traits.accessory < 12 ? accessoryRearSvg(traits.accessory) : expansion.accessories[traits.accessory - 12].rearSvg ?? "")}${bodySvg(body.color, body.shade, tokenId).replace(/dur="[\d.]+s"/g, `dur="${expressionDurations[traits.expression]}s"`)}${traits.expression === 13 ? diamondRays((tokenId % 36 * 7 + 3) % 6) + diamondRays(6 + (Math.floor(tokenId / 6) % 6 * 5 + 1) % 6) : ""}${animatedExpressionSvg(traits.expression)}${(traits.accessory === 20 ? royal.REGALIA : traits.accessory < 12 ? ACCESSORY_SVGS[traits.accessory] : expansion.accessories[traits.accessory - 12].svg)}${accessoryLabEffects(traits.accessory)}</g></g></g>${tokenBadgeSvg(tokenId, traits.background)}${compositionWatermark(traits)}`;
  ...
  if (traits.body === 16) scene = scene.replace(/<linearGradient id="bk-peel"[\s\S]*?<\/linearGradient>/, royal.PEEL);
