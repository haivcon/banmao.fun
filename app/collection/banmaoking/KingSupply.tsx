...
export default function KingSupply({ lang }: { lang: Lang }) {
  ...
  return <section className="king-supply" aria-label={t.collection}>
    <div className="king-supply-detail">
      <div className="king-supply-heading"><div className="king-supply-count"><strong>{data ? number.format(data.supply) : '—'}</strong><span>{minted}</span></div><span className="king-supply-percent">{progress}</span></div>
      ...
      <div className="king-supply-meta" title={data ? `${maximum}: ${number.format(data.max)}` : undefined}><span>{data ? `${number.format(data.max > data.supply ? data.max - data.supply : 0n)} ${remaining}` : t.loading}</span></div>
    </div>
    <div className="king-supply-fact"><strong>{number.format(TOTAL_COMBINATIONS)}</strong><span>{t.combinations}</span><small>{number.format(4)} · {t.layers}</small></div>
    <div className="king-supply-fact"><strong>{price} <span className="king-supply-currency">BANMAO</span></strong><span>{t.price} / NFT</span><small>{percent.format(1)} · {t.onchain}</small></div>
    <div className="king-supply-actions"><a className="king-primary-link" href="#king-mint">{t.mint} ↗</a><a className="king-secondary-link" href="#king-studio">{t.explore} ↓</a></div>
