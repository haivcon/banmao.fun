...
export function miniTraits(tokenId: number, slot: number): BanmaoKingTraitSelection {
  ...
  let accessory = Number((seed >> 128n) % 22n) + 1;
  if (accessory >= 22) accessory++;
  return { body: Number(seed % 15n), expression: Number((seed >> 64n) % 21n), accessory, background: 0 };
}

export function miniBanmaoSvg(tokenId: number, render: (traits: BanmaoKingTraitSelection, slot: number) => string): string {
  return [0, 1].map(slot => {
