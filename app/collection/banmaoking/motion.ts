...
import { whiskersSvg } from './expression-design';
import { loveEyesMotion } from './love-motion';
import { winkExpression } from './wink-motion';
import { sleepyExpression } from './sleepy-motion';
import { surprisedExpression } from './surprised-motion';
import { wateryTears } from './expression-effects';
import { resolveExpression } from './resolve-motion';
import { composedExpression } from './composed-motion';
...
function mouthMotion(svg: string, id: number) {
  ...
  const peaks: Record<string, string> = {
    ...
    'M244 267q12 12 24 0': 'M244 267q12 23 24 0',
...
export function animatedExpressionSvg(id: number) {
  ...
  if (id === 15) return whistlingExpression(whiskersSvg(), true);
  if (id === 16) return suspiciousExpression(whiskersSvg(), true);
  if (id === 17) return angryExpression(whiskersSvg(), true);
  if (id === 2) return winkExpression(expressionSvg(id));
  if (id === 4) return sleepyExpression(expressionSvg(id));
  if (id === 5) return surprisedExpression(expressionSvg(id));
  if (id === 6 || id === 7) return resolveExpression(expressionSvg(id), id);
  if (id === 9 || id === 10 || id === 11) return composedExpression(expressionSvg(id), id);
  ...
  const svg = mouthMotion(expressionSvg(id), id).replace(/<ellipse\b[^>]*fill="#ef8b8b"[^>]*opacity=".3"[^>]*\/>/g, tag =>
    tag.slice(0, -2) + ` class="king-blush"><animate attributeName="opacity" values="${id === 3 ? '.3;.78;.3' : '.3;.44;.3'}" keyTimes="0;.5;1" calcMode="spline" keySplines=".4 0 .6 1;.4 0 .6 1" dur="5.2s" repeatCount="indefinite"/></ellipse>`);
  ...
  if (id === 3) {
    eyes = loveEyesMotion(eyes);
  } else if (id === 4 || id === 11 || id === 19) {
    ...
    const wink = ''; // Wink owns its complete face timeline above.
