...
export default function KingComposition({ traits, onSelect, lang }: { traits: BanmaoKingTraitSelection; onSelect: (traits: BanmaoKingTraitSelection) => void; lang: Lang }) {
  ...
  const preview = useKingPreview(traits, 0, 'composition-export', exportRequested);
  useEffect(() => {
  ...
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" class="king-art">${preview.markup}</svg>`;
  return <div className="king-mint-box">
    ...
    <p>{preview.markup ? <a href={'data:image/svg+xml;charset=utf-8,' + encodeURIComponent(svg)} download={`${code}-preview.svg`}>{kingControl(lang, "Download preview SVG")}</a> : <button type="button" disabled={exportRequested && !preview.failed} onClick={() => { setExportRequested(true); if (preview.failed) preview.retry(); }}>{preview.failed ? KING_T[lang].retry : exportRequested ? KING_T[lang].processing : kingControl(lang, "Download preview SVG")}</button>}</p>
  </div>;
}
