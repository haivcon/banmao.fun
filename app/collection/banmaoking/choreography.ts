...
export function choreographySvg(expression: number): string {
  ...
  const wrists = (['left', 'right'] as const).map(side => {
    ...
    return (side === 'right' ? `<animateTransform href="#smil-king-staff-bob" attributeName="transform" type="translate" values="${beats.split(';').map(v => `0 ${v}`).join(';')}"${sideTiming(false)}/>` + rotate('king-held-wrist', angles, '0 -12') + rotate('king-shield-counter-wrist', inverse(angles), '369 357') : '') + rotate(`king-wrist-${side}`, angles, '0 -12') + shapes;
  }).join('');
