...
import { expandedTraitLabels } from './trait-expansion';
import { KING_T } from './index';
import { BODY_TRAITS, EXPRESSION_TRAITS, ACCESSORY_TRAITS, BACKGROUND_TRAITS } from '../traits';
...
id: [["Pisang emas","Mentari matang","Pisang limau","Pisang persik","Baju Cyborg"],["Senyum bahagia","Gembira","Kedipan","Mata cinta","Mengantuk","Terkejut","Bertekad","Berkaca-kaca","Jenaka","Tatapan keren","Mata bintang","Tenang"],["Tanpa aksesori","Mahkota raja","Pita merah","Kacamata bulat","Kacamata piksel","Topi pesta","Senapan AK","Pedang terbang","Headphone","Topi penyihir","Lingkar cahaya","Lentera bintang Vietnam"],["Krim pisang","Nexus Cyberpunk","Gelembung mint","Blok kerajaan","Kisi tengah malam","Ledakan mangga","Cincin permen","Awan biru"]],
};


// Complete each group without sparse slots or English fallback labels.
for (const lang of Object.keys(traitLabels) as Lang[]) {
  const groups = traitLabels[lang].map((group, index) => [...group, ...expandedTraitLabels[lang][index]]);
  groups[2].unshift(''); // Labels are keyed by public accessory ID, not array position.
  traitLabels[lang] = groups;
}

const canonicalGroups: readonly (readonly string[])[] = [BODY_TRAITS.map(v => v.name), EXPRESSION_TRAITS, ACCESSORY_TRAITS, BACKGROUND_TRAITS.map(v => v.name)];
const traitKeys = ['body', 'expression', 'accessory', 'background'] as const;
/** Presentation only: never mutate metadata used for identity or artwork. */
export function localizedKingAttribute(lang: Lang, attribute: { trait_type: string; value: string }) {
  const group = traitKeys.findIndex(key => key === attribute.trait_type.toLowerCase());
  if (group < 0) return { label: attribute.trait_type, value: attribute.value };
  const index = canonicalGroups[group].indexOf(attribute.value);
  return {
    label: KING_T[lang][traitKeys[group]],
    value: index < 0 ? attribute.value : traitLabels[lang][group][index + (group === 2 ? 1 : 0)],
  };
}
