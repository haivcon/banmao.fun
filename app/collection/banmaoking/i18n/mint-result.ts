...
export const MINT_RESULT_COPY: Record<Lang, ResultCopy> = {
  zh: {
    states: {
      ...
      replaced: ['交易已取消或替换', '原交易已被替换。再次操作前请在区块链浏览器查看替换交易；可能产生网络费用。'],
  ...
  ko: {
    states: {
      ...
      replaced: ['거래 취소 또는 교체', '원래 거래가 교체되었습니다. 다시 시도하기 전에 탐색기에서 교체 거래를 확인하세요. 네트워크 수수료가 발생할 수 있습니다.'],
  ...
  ru: {
    states: {
      ...
      replaced: ['Транзакция отменена или заменена', 'Исходная транзакция заменена. Проверьте замену в обозревателе перед повтором; возможна комиссия сети.'],
  ...
  id: {
    states: {
      ...
      replaced: ['Transaksi dibatalkan atau diganti', 'Transaksi awal diganti. Periksa transaksi pengganti di Explorer sebelum mencoba lagi; biaya jaringan mungkin berlaku.'],
  ...
  vi: {
    states: {
      ...
      replaced: ['Giao dịch đã bị hủy hoặc thay thế', 'Giao dịch ban đầu đã được thay thế. Kiểm tra giao dịch thay thế trên Explorer trước khi thử lại; phí mạng có thể phát sinh.'],
  ...
  en: {
    states: {
      ...
      replaced: ['Transaction cancelled or replaced', 'The original transaction was replaced. Inspect the replacement on Explorer before retrying; network fees may apply.'],
