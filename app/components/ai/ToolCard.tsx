...
export default function ToolCard({ tool }: { tool: ToolActivity }) {
  ...
  return <details className={`banmao-ai-tool ${failed ? "is-error" : complete ? "is-complete" : "is-running"}`} open={running}>
    <summary>
      ...
      <span><strong>{tool.name.replace(/[._-]+/g, " ")}</strong><small>{tool.source || "BANMAO service"}</small></span>
      <span className="banmao-ai-tool-status">{running ? "Working" : failed ? "Needs attention" : "Complete"}</span>
    </summary>
