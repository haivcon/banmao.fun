...
- BanmaoBox deployment availability is read from the checked-in X Layer mainnet manifest. BanMaoPK is unavailable because its only checked-in address is explicitly X Layer Testnet and no chain-196 deployment manifest exists. Collection search/prompts use bounded Cloudinary readers, and collection quests use SELECT-only Hub database reads; failures remain typed unavailable rather than mock data.
